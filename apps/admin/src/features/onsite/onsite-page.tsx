...
export function OnsitePage() {
  ...
  const load = useCallback(async () => {
    if (!token) return;
    try {
      const res = await fetchOnsite(token, filter || undefined);
      setData(res);
    } catch {
  ...
  // Reload full list on filter change or when SSE count changes
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-semibold text-pmg-navy">Host</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {data?.occupants.map((occ) => (
                <tr key={occ.personId}>
                  ...
                  <td className="px-4 py-3 text-gray-500 text-xs font-mono">{occ.lastLocationId}</td>
                  <td className="px-4 py-3 text-gray-500">{occ.host ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
