...
|------|-----|---------------------------|
| **Capacitor wrapper (iOS)** | Reliable background sync/storage for marshals on the employee app on iPhones (PWA can't) | PWA built; Capacitor wraps the same web app — planned stack |
